...
def create_image_dir():
    ...
    for url in urls:
         ...
         try:
             ...
             img.save(f'C:/Users/camer/PycharmProjects/WindowPractice/Tarkov Images/{url[1]}.png')
         except:
             print(f'--------ITEM FAILURE ON IMAGE LOAD--------{url[1]} @ {url[0]}')
class Tarkov_Item():
    def __init__(self,name,price,category,image=None,):
        ...
        try:
            self.image = cv2.cvtColor(np.array(Image.open(f'{self.name}.png')), cv2.COLOR_RGB2BGR)
        except:
            self.image = cv2.cvtColor(np.array(Image.open('zzzzErrorImage.png')),cv2.COLOR_RGB2BGR)
...
def object_creation(json_file):
    ...
    for items in json_file:
        ...
        if all_info[0][2][0] == 'Barter':
            tarkov_item = Tarkov_Item(all_info[0][1],all_info[0][4],all_info[0][2][0])
...
lookup_button = Button(utility_frame,text = 'Start',command = lambda: image_lookup_in_image(tarkovAllItems))
lookup_button.grid(row = 5,column = 0)
clear_button = Button(utility_frame,text = 'clear',command = clear_Window)
update_button = Button(utility_frame,text = 'Update images',command = create_image_dir)
